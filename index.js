...
Promise.all([mandelbrot]).then(async function ([{ generate_mandelbrot_set, draw_mandelbrot_set }]) {
    console.log("fisish loading wasm");
    ...
    renderBtn.addEventListener('click', () => {
        ...
        let wasmResult = null;
        {
            ...
            const generateStartTime = Data.now();
            wasmResult = generate_mandelbrot_set(canvasWidth, canvasHeight, X_MIN, X_MAX, Y_MIN, Y_MAX, MAX_ITER);
            const generateEndTime = Data.now();
            const drawStartTime = Data.now();
            draw(context, canvasWidth, canvasHeight, wasmResult);
            const drawEndTime = Date.now();
            const elapsed = generateEndTime - generateStartTime;
            console.log(`\tgenerate:wasm\tgenerete_elapsed:${elapsed}[ms]`);
            console.log(`\tdraw: js\tdraw_elapsed: ${drawEndTime - drawEndTime} [ms]`);
        }
    });
});
